 ...
 */
function logAsanaChangeHeader(reqHeaders, resHeaders) {
  if (resHeaders && resHeaders['asana-change']) {
    ...
    for (var header in reqHeaders) {
      if (!reqHeaders.hasOwnProperty(header)) {
        continue;
      }

      if (header.toLowerCase() === 'asana-enable') {
        reqHeaders[header].split(',').forEach(addFlag);
      } else if (header.toLowerCase() === 'asana-disable') {
        reqHeaders[header].split(',').forEach(addFlag);
      }
    }

    var changes = resHeaders['asana-change'].split(',');
    changes.forEach(function(unsplitChange) {
      ...
      // asana-enable header
      if (!accountedForFlags.includes(name) && affected === 'true') {
    ...
    function doRequest() {
      ...
      request(params, function(err, res, payload) {
        ...
        if (me.logAsanaChangeWarnings) {
          logAsanaChangeHeader(params.headers, res.headers);
        }
