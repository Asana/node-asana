...
function Dispatcher(options) {
   ...
   * @type {String}
   */
  this.ignoreAsanaChange = options.ignoreAsanaChange || false;
}
  ...
  return new Bluebird(function(resolve, reject) {
    function doRequest() {
      ...
      request(params, function(err, res, payload) {
        ...
        // Handle Asana-Change headers
        if (!me.ignoreAsanaChange && res.headers &&
            res.headers['asana-change']) {
          var asanaEnabledFlags = [];

          // Grab the request's asana-enable flags
          if (params.headers['asana-enable']) {
            var flags = params.headers['asana-enable'].split(',');
            flags.forEach(function(flag) {
              asanaEnabledFlags.push(flag.trim());
            });
          }

          var changes = res.headers['asana-change'].split(',');
          changes.forEach(function(unsplitChange) {
            ...
            for (var j = 0; j < change.length; j++) {
              ...
              if (field[0].trim() === 'info') {
                info = field[1].trim();
              }
              if (field[0].trim() === 'affected') {
            ...
            // asana-enable header
            if (!asanaEnabledFlags.includes(name) && affected === 'true') {
              console.error('This request is affected by the "' + name +
                  '" deprecation. Please visit this url to resolve it: ' +
                  info);
            }
          });
        }
